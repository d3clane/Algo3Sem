...
struct Line
{
  ...
  explicit Line(int slopeFactor = 0, int yOffset = 0) : slopeFactor(slopeFactor), yOffset(yOffset) {}
...
void LowerEnvelope::addLine(const Line& line)
{
  ...
  while (lines_.size() != 0 && lines_.back().getY(intersectionX) > line.getY(intersectionX))
  {
    lines_.pop_back();
...
{
  int lineIndex = std::lower_bound(linesIntersectionsX_.begin(), linesIntersectionsX_.end(), x) - 
...
int findMinSquareLens(const int n, const int k)
{
  ...
  for (int numberOfSegments = 1; numberOfSegments <= k; ++numberOfSegments)
  {
    ...
    {
      dp[pointId][numberOfSegments] = envelope.getY(pointId) + pointId * pointId;
      newEnvelope.addLine(
        Line{-2 * (pointId + 1), dp[pointId][numberOfSegments] + (pointId + 1) * (pointId + 1)}
      );
    }
